...
export async function updateEmployeePii(
  params: UpdateEmployeePiiParams,
  employerWallet: Address
): Promise<EmployeeIdentity> {
  const res = await fetch("/api/company/employee-identity", {
    method: "PATCH",
    headers: employerHeaders(employerWallet),
    body: JSON.stringify(params),
  });
  ...
  const json = await res.json();
  if (!res.ok) throw new Error(json?.error?.message ?? "Failed to update employee identity");
  return (json.identity ?? json) as EmployeeIdentity;
}
...
export async function syncEmployeeToCompany(params: {
  ...
  chain_id: number;
  active: boolean;
}, employerWallet: Address): Promise<void> {
