...
export function SingleEmployeePayrollModal({
  ...
  const checks: ContractCheck[] = [
    ...
    },
    {
      ...
      ok: operatorOk,
      detail: operatorOk
        ? `The payroll contract is approved as an operator and can call operatorTransferEncrypted() on your behalf.`
        : `The payroll contract is NOT approved as an operator. The transaction will revert with PayrollNotOperator. Grant approval in Operator Controls before running payroll.`,
    },
    {
  ...
  return (
    <div className="fixed inset-0 z-50">
      {/* Backdrop */}
      <button
        className="absolute inset-0 bg-[rgba(2,8,23,0.68)] backdrop-blur-md"
        aria-label="Close"
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div className="w-full max-w-xl overflow-hidden rounded-[32px] border border-white/15 bg-[rgba(248,250,252,0.92)] shadow-[0_30px_120px_rgba(2,8,23,0.45)] backdrop-blur-2xl dark:bg-[rgba(7,18,36,0.94)]">

          {/* Header */}
          <div className="flex items-start justify-between gap-4 border-b border-white/15 p-6">
            <div>
              <div className="section-label">Single employee run</div>
              <h2 className="mt-2 text-3xl font-semibold tracking-tight">Run payroll for employee</h2>
          ...
          <div className="max-h-[70vh] space-y-4 overflow-y-auto p-6">
            ...
            {/* Employee summary */}
            <div className="rounded-[24px] border border-white/20 bg-white/70 p-5 space-y-2 dark:border-white/10 dark:bg-white/5">
              <p className="section-label">Employee</p>
              <div className="flex items-center justify-between gap-3">
                <code className="text-xs font-mono text-foreground">
                  {employee.wallet_address}
                </code>
                <Badge variant="outline" className="shrink-0 rounded-full px-3 py-1 text-[10px]">
            ...
            <div className="rounded-[24px] border border-white/20 bg-white/70 p-5 space-y-3 dark:border-white/10 dark:bg-white/5">
              <p className="section-label">Contract requirements</p>
              {checks.map((check) => (
                <div key={check.label} className={`rounded-[20px] p-4 ${check.ok ? "bg-success/10 border border-success/20" : "bg-destructive/10 border border-destructive/20"}`}>
            ...
            {/* What will happen */}
            <div className="rounded-[24px] border border-white/20 bg-white/70 p-5 space-y-2 dark:border-white/10 dark:bg-white/5">
              <p className="section-label">What this will do</p>
              <p className="text-xs text-muted-foreground leading-relaxed">
                ...
                The contract will execute <code>operatorTransferEncrypted(employer, employee, encryptedSalary)</code> —
            ...
            {/* Acknowledgements */}
            <div className="rounded-[24px] border border-white/20 bg-white/70 p-5 space-y-3 dark:border-white/10 dark:bg-white/5">
              <p className="section-label">Confirm before running</p>
          ...
          {/* Footer */}
          <div className="space-y-3 border-t border-white/15 p-6">
            {txError && (
              <div className="rounded-[20px] border border-destructive/30 bg-destructive/10 px-4 py-3">
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              >
                {busy ? "Submitting…" : "Run employee payroll"}
              </Button>
            </div>
          </div>
