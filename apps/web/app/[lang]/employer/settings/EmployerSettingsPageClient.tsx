...
export function EmployerSettingsPageClient() {
    ...
    <EmployerPageShell currentPath={`/${ctx.locale}/employer/settings`}>
      <Card className="border-white/40 bg-white/74 dark:border-white/10 dark:bg-[rgba(7,18,36,0.76)]">
        <CardHeader className="pb-2">
          <CardTitle className="section-label">Company</CardTitle>
        </CardHeader>
        <CardContent className="space-y-0">
          <InfoRow label="Legal name" value={ctx.companyName} />
          <InfoRow label="Backend sync" badge={{ text: ctx.backendSynced ? "Synced" : "Not synced", ok: ctx.backendSynced }} />
          {ctx.companyOnchainBindingId && <AddressRow label="On-chain binding ID" value={ctx.companyOnchainBindingId} />}
        </CardContent>
      </Card>

      <Card className="border-white/40 bg-white/74 dark:border-white/10 dark:bg-[rgba(7,18,36,0.76)]">
        <CardHeader className="pb-2">
          <CardTitle className="section-label">Wallet & Network</CardTitle>
        </CardHeader>
        <CardContent className="space-y-0">
          <AddressRow label="Employer wallet" value={me} />
          <InfoRow label="Chain" value={`${chainNames[chainId] ?? "Unknown"} (${chainId})`} />
          <InfoRow
            label="FHE available"
      ...
      <Card className="border-white/40 bg-white/74 dark:border-white/10 dark:bg-[rgba(7,18,36,0.76)]">
        <CardHeader className="pb-2">
          <CardTitle className="section-label">Contract Addresses</CardTitle>
        </CardHeader>
        <CardContent className="space-y-0">
          ...
          {!ctx.hasCompany && <p className="text-xs text-muted-foreground pt-2">No payroll contract deployed yet for this wallet.</p>}
        </CardContent>
      </Card>

      <Card className="border-white/40 bg-white/74 dark:border-white/10 dark:bg-[rgba(7,18,36,0.76)]">
        <CardHeader className="pb-2">
          <CardTitle className="section-label">Token Configuration</CardTitle>
        </CardHeader>
        <CardContent className="space-y-0">
      ...
      {ctx.hasCompany && <DeleteCompanyButton companyName={ctx.companyName} onDeleteCompany={ctx.onDeleteCompany} />}
    </EmployerPageShell>
  );
}
