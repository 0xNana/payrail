...
export async function POST(req: Request) {
  try {
    ...
    // ── Reuse existing person_wallet when the wallet already exists ──────────
    const { data: existingWallet, error: existingWalletErr } = await supabaseAdmin
      .from("person_wallet")
      .select("person_wallet_id, person_id")
      .eq("wallet_address", walletNorm)
      .eq("chain_id", Number(chain_id))
      .maybeSingle();

    if (existingWalletErr) {
      return NextResponse.json({ error: existingWalletErr }, { status: 500 });
    }
    ...
    if (existingWallet) {
      person_id = existingWallet.person_id;
      person_wallet_id = existingWallet.person_wallet_id;

      const { error: activateWalletErr } = await supabaseAdmin
        .from("person_wallet")
        .update({ active: true })
        .eq("person_wallet_id", person_wallet_id);
      if (activateWalletErr) {
        return NextResponse.json({ error: activateWalletErr }, { status: 500 });
      }

      const { data: existingIdentity, error: existingIdentityErr } = await supabaseAdmin
      ...
      if (existingIdentityErr) {
        return NextResponse.json({ error: existingIdentityErr }, { status: 500 });
      }
      ...
      const { data: walletRow, error: walletErr } = await supabaseAdmin
        .from("person_wallet")
        .insert({ person_id, wallet_address: walletNorm, chain_id: Number(chain_id), active: true })
        .select("person_wallet_id")
        .single();

      if (walletErr || !walletRow) {
        await supabaseAdmin.from("person").delete().eq("person_id", person_id);
        return NextResponse.json({ error: walletErr ?? { message: "Failed to create person_wallet" } }, { status: 500 });
      }
    ...
    if (employmentErr || !employmentRow) {
      if (createdPersonId) await supabaseAdmin.from("person").delete().eq("person_id", createdPersonId);
    ...
    if (ecbErr) {
      await supabaseAdmin.from("employment").delete().eq("employment_id", employment_id);
      if (createdPersonId) await supabaseAdmin.from("person").delete().eq("person_id", createdPersonId);
