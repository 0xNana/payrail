...
if (!chainId) {
  chainId = networkChainIds[network] ?? 0;
}
...
const deploymentFiles = fs.existsSync(deploymentsDir)
  ? fs
...
if (!fs.existsSync(deploymentsDir)) {
  console.log(`   No deployments directory found at ${deploymentsDir}; generated ABI-only SDK snapshot.`);
}
console.log(
